...
class OverallReportController extends Controller {
    ...
    public function indexAction(Request $request) {
        ...
        $statement = $connection->prepare("select district, gender, (case 
                                     ...
                                     ) as t, sum(hours) from VolunteerContactDetails join VolunteerPersonal join Profile 
                                     on VolunteerPersonal.user=VolunteerContactDetails.user and VolunteerContactDetails.user=Profile.user 
                                     group by district, gender, t order by district, gender, t");
        ...
        $statement = $connection->prepare("select occupation, gender, (case 
                                     ...
                                     ) as t, sum(hours) from Employment join VolunteerPersonal join Profile 
                                     on VolunteerPersonal.user=Employment.user and Employment.user=Profile.user 
                                     group by occupation, gender, t order by occupation, gender, t");

        $statement->execute();
        $hoursByProfession = $statement->fetchAll();

        $statement = $connection->prepare("select gender, (case 
                          ...
                          when datediff(current_date, str_to_date(dateofbirth,'%m/%d/%Y'))/365 between 50 and 60 then '50-60'
                                     ...
                                     ) as t, sum(hours) from VolunteerPersonal join Profile on VolunteerPersonal.user=Profile.user group by  gender, t order by  t,gender");
        ...
        $statement = $connection->prepare("select district, gender, (case 
                                     ...
                                     ) as t, sum(hours*value) from VolunteerContactDetails join VolunteerPersonal join Profile 
                                     on VolunteerPersonal.user=VolunteerContactDetails.user and VolunteerContactDetails.user=Profile.user 
                                     group by district, gender, t order by district, gender, t");
        ...
        $statement = $connection->prepare("select occupation, gender, (case 
                                     ...
                                     ) as t, sum(hours*value) from Employment join VolunteerPersonal join Profile 
                                     on VolunteerPersonal.user=Employment.user and Employment.user=Profile.user 
                                     group by occupation, gender, t order by occupation, gender, t");
        ...
        $statement = $connection->prepare("select gender, (case 
                          ...
                          when datediff(current_date, str_to_date(dateofbirth,'%m/%d/%Y'))/365 between 50 and 60 then '50-60'
                                     ...
                                     ) as t, sum(hours*value) from VolunteerPersonal join Profile on VolunteerPersonal.user=Profile.user group by  gender, t order by  t,gender");
        ...
        $pdf->SetXY(25.4, 100);
        $pdf->MultiCell(0, 6, 'OVERALL VOLUNTEER REPORT', 0, 'L');
        ...
        //return $this->render('IsuruStatBundle:Reports:overall.html.twig', array(
        //          'list' => $list));
    }
